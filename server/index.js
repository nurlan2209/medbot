...
const mongouri = 'mongodb+srv://nurlank1234554321_db_user:Nurlan11@cluster0.mm4qxdj.mongodb.net/medbot_db?retryWrites=true&w=majority&appName=Cluster0';

mongoose.connect(mongouri)
  .then(() => console.log('mongodb подключен успешно'))
  .catch((err) => console.error('ошибка подключение к монгодб:', err));

const GEMINI_API_KEY = process.env.GEMINI_API_KEY || 'AIzaSyAws5XdEpe6fUIDrlxUzxTr8hWqC1qrnDU';
...
app.post('/register', async (req, res) => {
  try {
    ...
    res.status(201).send({ message: 'Регистрация успешно', user: userResponse });
  } catch (error) {
    if (error.code === 11000) {
...
app.post('/login', async (req, res) => {
  try {
    ...
    delete userResponse.password;
    res.status(200).send({ message: 'вход выполнен', user: userResponse });
  } catch (error) {
...
app.post('/upload-avatar/:email', upload.single('avatar'), async (req, res) => {
  try {
